...
public class Asset<T extends Asset<T>> extends Model<T> {
	...
	public String getTrangThaiTraLoi() {
		return trangThaiTraLoi;
	}

	public void setTrangThaiTraLoi(String trangThaiTraLoi) {
		this.trangThaiTraLoi = Strings.nullToEmpty(trangThaiTraLoi);
	}
	...
	@Transient
	public String setStyleTrangThaiTraLoi( String strStyle) {
		if (core().TT_CHUA_TRA_LOI.equals(strStyle)) {
			return "label label-default width-110px";
		} else if (core().TT_DA_TRA_LOI.equals(strStyle)) {
			return "label label-success width-90px";
		} else {
			return "";
		}
	...
	public String getTrangThaiTraLoiText() {
		return new BasicService<>().getTrangThaiTraLoiList().get(getTrangThaiTraLoi());
	}
